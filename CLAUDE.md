...
| `pages/PostDetail.tsx` | Single post page. Uses `useGetPostQuery` and `useGetRelatedPostsQuery` hooks, renders Markdown content via `react-markdown` with prose styling. Shows "Related Posts" section (tag-overlap ranked) and `CommentSection` below the article. Sets `<title>`, description (from excerpt), and OG article tags. Shows Edit/Delete buttons when admin is logged in (hybrid admin controls). |
